...
# --- Список городов (ALL_CITIES) и тестовых чатов ---
ALL_CITIES = [
    ...
    {"name": "Тула",           "link": "https://t.me/+ZCq3GsGagIQ1NzRi", "chat_id": -1002678281080},  # <-- Добавили Тулу
]
...
def menu(update: Update, context: CallbackContext):
    ...
    if uid not in ALLOWED_USER_IDS:
        return update.message.reply_text("У вас нет прав для использования бота.")
    keyboard = [
...
def handle_main_menu(update: Update, context: CallbackContext):
    ...
    if choice == "Тестовая отправка":
        ...
        update.message.reply_text(
            "Введите тестовое сообщение (текст или медиа).\nЧтобы отменить — /menu",
            disable_web_page_preview=True
        )
        raise DispatcherHandlerStop
    ...
    if context.user_data.pop("pending_test", False):
        ...
        for cid in TEST_SEND_CHATS:
            try:
                ...
                failures.append(cid)
        reply = (
            "Не удалось в: " + ", ".join(map(str, failures))
    ...
    if "selected_chats" in context.user_data:
        ...
        for cid in chat_ids:
            try:
                ...
                failures.append(cid)
        reply = (
            "Не отправлено в: " + ", ".join(map(str, failures))
